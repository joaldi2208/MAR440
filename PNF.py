...
def read_in():
    n_data = pd.read_csv("Data/data0.tsv", sep="\t")
    p_data = pd.read_csv("Data/data1.tsv", sep="\t")
    ctd_data = pd.read_csv("Data/CTD-Data.tsv", sep="\t")
    ...
    plots(original_stations, group_depth, rows, n_data, p_data)
    
def plots(original_stations, group_depth, rows, n_data, p_data):
    for i in range(6):
        fig = plt.figure()
        ...
        ax2.set_xlabel("Concentration [$\mu$mol/L]")
        ax.invert_yaxis()
        ...
        plt.savefig(f"Images/F_N_P_station{i+1}.png", dpi=300, bbox_inches="tight")
        plt.show()
